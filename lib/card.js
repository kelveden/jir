...
module.exports = function (config) {
    const cardColour = config.colour;
    const wrap = require('wordwrap')(cardWidth - 4);
    ...
    this.render = function (rows) {
        ...
        function renderRow(row, index) {
            function renderBottom(nextRow) {
                ...
                if (!nextRow) {
                    ...
                    const internals = _.union(row.separators, nextRow.separators, [cardWidth]).sort().reduce(function (line, separatorIndex) {
                        let separatorChar = chars.internal.bothboth;
            ...
            const nextRow = rows.length > (index + 1) ? rows[index + 1] : null
            const rowHeight = _.max(row.map(function (col) {
            ...
            for (let i = 0; i < rowHeight; i++) {
                console.log(
                    ...
                    row.map(function (col) {
                        const content = col.content.length > i ? col.content[i] : "";
        ...
        function setLastColumnWidth(row) {
            const width = lastColumnWidth(row);
